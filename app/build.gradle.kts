...
// AI provider keys from local.properties (gitignored). Comma-separated list lets
// the runtime ApiKeyPool rotate across multiple keys from different Google
// projects — one expired or rate-limited key never stalls the chain.
//
//   GEMINI_API_KEYS=AIza...key1,AIza...key2
//
// Backward compatibility: the legacy single-key form `GEMINI_API_KEY=...` is
// still accepted as a 1-element list when the plural form is absent —
// onboarding users who paste a single key from Google AI Studio shouldn't have
// to learn the pluralized name to get a working build. Falls back to empty so
// the app still builds without secrets; AI features then surface a clear error
// category at runtime.
//
